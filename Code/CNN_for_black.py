...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #X = X.unsqueeze(0)
        #X = normalize(X) #<---
        #X = torch.nn.functional.normalize(X, p=2.5)
...
def model_definition(pretrained=False):
    ...
    if pretrained == True:
        ...
        model.classifier = nn.Linear(num_ftrs, OUTPUTS_a)
    ...
    criterion = nn.CrossEntropyLoss() #acc .30 for resnet50
    scheduler = ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=0, verbose=True)

    save_model(model)

    return model, optimizer, criterion, scheduler # add scheduler here
#------------------------------------------------------------------------------------------------------------------


def train_and_test(train_ds, test_ds, list_of_metrics, list_of_agg, save_on, pretrained = False):

    model, optimizer, criterion, scheduler = model_definition(pretrained) #add scheduler here
    ...
    for epoch in range(n_epoch):
        ...
        for i in range(len(PRED_labels)):
            res = [1 if item == max(PRED_labels[i]) else 0 for item in PRED_labels[i]]
...
if __name__ == '__main__':
    ...
    train_and_test(train_ds, test_ds, list_of_metrics, list_of_agg, save_on='acc', pretrained=True)
